...
func TestMetricsValues(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if strings.Count(output, "targetPort: metrics") != tc.services || strings.Count(output, "kind: ServiceMonitor") != tc.monitors {
				t.Fatal("unexpected service/monitor count")
			}
